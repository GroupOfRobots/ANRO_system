...
def load_environment(client_id):
    ...
    robot_model_path = path.abspath(path.join(__file__ ,"../../..")) + "/dobot_description/meshes/collision/urdf_models/dobot_with_rail.urdf"
    ...
    # for i in range(pyb.getNumJoints(dobot_magician_id)):
    #     print(pyb.getJointInfo(dobot_magician_id, i))

    #TODO urdf ground plane
    # ground plane
    sliding_rail_model_path = path.abspath(path.join(__file__ ,"../../..")) + "/dobot_description/meshes/collision/urdf_models/sliding_rail_draft.urdf"
    rail_id = pyb.loadURDF(
        sliding_rail_model_path, [0, 0.5, -0.161305], useFixedBase=True, physicsClientId=client_id
    )
    ...
    ground_id = pyb.loadURDF(
        "plane.urdf", [0, 0, -0.191305], useFixedBase=True, physicsClientId=client_id
    )
...
def create_robot_debug_params(robot_id, gui_id):
    ...
    params['magician_joint_base'] = pyb.addUserDebugParameter('magician_joint_base', 
                                                        rangeMin = 0, 
                                                        rangeMax = 1, 
                                                        startValue=0, 
...
def read_robot_configuration(robot_id, robot_params, gui_id):
    ...
    q[4] = pyb.readUserDebugParameter(robot_params['magician_joint_base'], physicsClientId=gui_id)
...
def update_robot_configuration(robot_id, q, gui_id):
    ...
    pyb.resetJointState(robot_id, 0, q[4], physicsClientId = gui_id)
...
def main():
    ...
    ground = NamedCollisionObject("ground")
    rail = NamedCollisionObject("rail")

    col_detector = CollisionDetector(
        col_id,
        collision_bodies,
        # [(link7, ground), (link7, cube1), (link7, cube2), (link7, cube3), (link7, base)],
        [(link7, base), (link7, ground), (link7, link1), (link7, rail), (link7_2, base), (link7_2, ground), (link7_2, link1), (link7_2, rail)],
    )
